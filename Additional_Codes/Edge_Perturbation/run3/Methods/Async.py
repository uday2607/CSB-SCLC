...
def SummaryAsync(nodes,inter_mat,input,IniState,FixedState,TurnState,folder):
    ...
    workbook = xlsxwt.Workbook(os.path.join('OUTPUT',folder,'Summary_Async.xlsx'))
    ...
    worksheet.write(0, len(nodes)+2,'Frequency')
    worksheet.write(0, len(nodes)+3,'Frustration')

    for i, (state,values) in enumerate(SteadyState.items()):
        worksheet.write(i+1,0,'Fixed point')
        vect = num2vect(state,len(nodes))
        for j,node in enumerate(vect):
            if node == input['node_values'][0]:
                worksheet.write(i+1,j+1,str(node),cell_format)
            else:
                worksheet.write(i+1,j+1,str(node))
        worksheet.write(i+1,len(nodes)+2,str(values))
        worksheet.write(i+1,len(nodes)+3,str(frustration[state]))

    workbook.close()

    with open(os.path.join('OUTPUT',folder,'states.f'), 'wb') as f:
        states, freq = np.array(list(SteadyState.keys())), np.array(list(SteadyState.values()))
        states = col2vect(states, len(nodes))
        SStates = np.vstack((states.T,freq)).T
        pickle.dump(SStates,f)

    with open(os.path.join('OUTPUT',folder,'intermat.f'), 'wb') as f:
        pickle.dump(inter_mat, f)     
